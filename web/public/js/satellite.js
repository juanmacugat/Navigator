...
satellite = (function() {
	...
	/*
	 * satellite-head.js and satellite-tail.js sandwich all the other functions
	 * in the library.
	 * 
	 * The exposed functions are returned out via the satellite object
	 * 
	 * This is to separate the satellite.js namespace from the rest of the
	...
	var xke = 60.0 / Math.sqrt(radiusearthkm * radiusearthkm * radiusearthkm
			/ mu);
	var tumin = 1.0 / xke;
	...
	function dpper(satrec, dpper_parameters) {
		 ...
		 * this used to be dscom which included initialization, but it's really
		 * a recurring function.
		 * 
		 * author : david vallado 719-573-2600 28 jun 2005
		 * 
		 * inputs : e3 - ee2 - peo - pgho - pho - pinco - plo - se2 , se3 ,
		 * sgh2, sgh3, sgh4, sh2, sh3, si2, si3, sl2, sl3, sl4 - t - xh2, xh3,
		 * xi2, xi3, xl2, xl3, xl4 - zmol - zmos - ep - eccentricity 0.0 - 1.0
		 * inclo - inclination - needed for lyddane modification nodep - right
		 * ascension of ascending node argpp - argument of perigee mp - mean
		 * anomaly
		 * 
		 * outputs : ep - eccentricity 0.0 - 1.0 inclp - inclination nodep -
		 * right ascension of ascending node argpp - argument of perigee mp -
		 * mean anomaly
		 * 
		 * locals : alfdp - betdp - cosip , sinip , cosop , sinop , dalf - dbet -
		 * dls - f2, f3 - pe - pgh - ph - pinc - pl - sel , ses , sghl , sghs ,
		 * shl , shs , sil , sinzf , sis , sll , sls xls - xnoh - zf - zm -
		 * 
		 * coupling : none.
		 * 
		...
		var inclo = dpper_parameters.inclo, init = dpper_parameters.init, ep = dpper_parameters.ep, inclp = dpper_parameters.inclp, nodep = dpper_parameters.nodep, argpp = dpper_parameters.argpp, mp = dpper_parameters.mp, opsmode = dpper_parameters.opsmode;

		// Copy satellite attributes into local variables for convenience
		// and symmetry in writing formulae.

		var alfdp, betdp, cosip, sinip, cosop, sinop, dalf, dbet, dls, f2, f3, pe, pgh, ph, pinc, pl, sel, ses, sghl, sghs, shl, shs, sil, sinzf, sis, sll, sls, xls, xnoh, zf, zm, shll;
	...
	function dscom(dscom_parameters) {
		...
		var epoch = dscom_parameters.epoch, ep = dscom_parameters.ep, argpp = dscom_parameters.argpp, tc = dscom_parameters.tc, inclp = dscom_parameters.inclp, nodep = dscom_parameters.nodep, np = dscom_parameters.np, e3 = dscom_parameters.e3, ee2 = dscom_parameters.ee2, peo = dscom_parameters.peo, pgho = dscom_parameters.pgho, pho = dscom_parameters.pho, pinco = dscom_parameters.pinco, plo = dscom_parameters.plo, se2 = dscom_parameters.se2, se3 = dscom_parameters.se3, sgh2 = dscom_parameters.sgh2, sgh3 = dscom_parameters.sgh3, sgh4 = dscom_parameters.sgh4, sh2 = dscom_parameters.sh2, sh3 = dscom_parameters.sh3, si2 = dscom_parameters.si2, si3 = dscom_parameters.si3, sl2 = dscom_parameters.sl2, sl3 = dscom_parameters.sl3, sl4 = dscom_parameters.sl4, xgh2 = dscom_parameters.xgh2, xgh3 = dscom_parameters.xgh3, xgh4 = dscom_parameters.xgh4, xh2 = dscom_parameters.xh2, xh3 = dscom_parameters.xh3, xi2 = dscom_parameters.xi2, xi3 = dscom_parameters.xi3, xl2 = dscom_parameters.xl2, xl3 = dscom_parameters.xl3, xl4 = dscom_parameters.xl4, zmol = dscom_parameters.zmol, zmos = dscom_parameters.zmos;

		var a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, betasq, cc, ctem, stem, x1, x2, x3, x4, x5, x6, x7, x8, xnodce, xnoi, zcosg, zsing, zcosgl, zsingl, zcosh, zsinh, zcoshl, zsinhl, zcosi, zsini, zcosil, zsinil, zx, zy;

		var ss1, ss2, ss3, ss4, ss5, ss6, ss7, sz1, sz2, sz3, sz11, sz12, sz13, sz21, sz22, sz23, sz31, sz32, sz33;
	...
	function dsinit(dsinit_parameters) {
		 ...
		 *    vallado, crawford, hujsak, kelso  2006
		   ----------------------------------------------------------------------------*/
		'use strict';
		var cosim = dsinit_parameters.cosim, emsq = dsinit_parameters.emsq, argpo = dsinit_parameters.argpo,

		s1 = dsinit_parameters.s1, s2 = dsinit_parameters.s2, s3 = dsinit_parameters.s3, s4 = dsinit_parameters.s4, s5 = dsinit_parameters.s5, sinim = dsinit_parameters.sinim,

		ss1 = dsinit_parameters.ss1, ss2 = dsinit_parameters.ss2, ss3 = dsinit_parameters.ss3, ss4 = dsinit_parameters.ss4, ss5 = dsinit_parameters.ss5,

		sz1 = dsinit_parameters.sz1, sz3 = dsinit_parameters.sz3, sz11 = dsinit_parameters.sz11, sz13 = dsinit_parameters.sz13, sz21 = dsinit_parameters.sz21, sz23 = dsinit_parameters.sz23, sz31 = dsinit_parameters.sz31, sz33 = dsinit_parameters.sz33,

		t = dsinit_parameters.t, tc = dsinit_parameters.tc, gsto = dsinit_parameters.gsto,

		mo = dsinit_parameters.mo, mdot = dsinit_parameters.mdot, no = dsinit_parameters.no, nodeo = dsinit_parameters.nodeo, nodedot = dsinit_parameters.nodedot,

		xpidot = dsinit_parameters.xpidot,

		z1 = dsinit_parameters.z1, z3 = dsinit_parameters.z3, z11 = dsinit_parameters.z11, z13 = dsinit_parameters.z13, z21 = dsinit_parameters.z21, z23 = dsinit_parameters.z23, z31 = dsinit_parameters.z31, z33 = dsinit_parameters.z33,

		ecco = dsinit_parameters.ecco, eccsq = dsinit_parameters.eccsq, em = dsinit_parameters.em,

		argpm = dsinit_parameters.argpm, inclm = dsinit_parameters.inclm, mm = dsinit_parameters.mm, nm = dsinit_parameters.nm, nodem = dsinit_parameters.nodem, irez = dsinit_parameters.irez, atime = dsinit_parameters.atime,

		d2201 = dsinit_parameters.d2201, d2211 = dsinit_parameters.d2211, d3210 = dsinit_parameters.d3210, d3222 = dsinit_parameters.d3222, d4410 = dsinit_parameters.d4410, d4422 = dsinit_parameters.d4422,

		d5220 = dsinit_parameters.d5220, d5232 = dsinit_parameters.d5232, d5421 = dsinit_parameters.d5421, d5433 = dsinit_parameters.d5433,

		dedt = dsinit_parameters.dedt, didt = dsinit_parameters.didt, dmdt = dsinit_parameters.dmdt, dnodt = dsinit_parameters.dnodt, domdt = dsinit_parameters.domdt,

		del1 = dsinit_parameters.del1, del2 = dsinit_parameters.del2, del3 = dsinit_parameters.del3,

		xfact = dsinit_parameters.xfact, xlamo = dsinit_parameters.xlamo, xli = dsinit_parameters.xli, xni = dsinit_parameters.xni;
		...
		var rptim = 4.37526908801129966e-3; // equates to 7.29211514668855e-5
											// rad/sec
	...
	function dspace(dspace_parameters) {
		...
		var irez = dspace_parameters.irez, d2201 = dspace_parameters.d2201, d2211 = dspace_parameters.d2211, d3210 = dspace_parameters.d3210, d3222 = dspace_parameters.d3222, d4410 = dspace_parameters.d4410, d4422 = dspace_parameters.d4422, d5220 = dspace_parameters.d5220, d5232 = dspace_parameters.d5232, d5421 = dspace_parameters.d5421, d5433 = dspace_parameters.d5433, dedt = dspace_parameters.dedt, del1 = dspace_parameters.del1, del2 = dspace_parameters.del2, del3 = dspace_parameters.del3, didt = dspace_parameters.didt, dmdt = dspace_parameters.dmdt, dnodt = dspace_parameters.dnodt, domdt = dspace_parameters.domdt, argpo = dspace_parameters.argpo, argpdot = dspace_parameters.argpdot, t = dspace_parameters.t, tc = dspace_parameters.tc, gsto = dspace_parameters.gsto, xfact = dspace_parameters.xfact, xlamo = dspace_parameters.xlamo, no = dspace_parameters.no, atime = dspace_parameters.atime, em = dspace_parameters.em, argpm = dspace_parameters.argpm, inclm = dspace_parameters.inclm, xli = dspace_parameters.xli, mm = dspace_parameters.mm, xni = dspace_parameters.xni, nodem = dspace_parameters.nodem, nm = dspace_parameters.nm;
		...
		if (irez !== 0) {
			...
			while (iretn === 381) {
				...
				if (irez !== 2) {
					xndt = del1 * Math.sin(xli - fasx2) + del2
							* Math.sin(2.0 * (xli - fasx4)) + del3
							* Math.sin(3.0 * (xli - fasx6));
					xldot = xni + xfact;
					xnddt = del1 * Math.cos(xli - fasx2) + 2.0 * del2
					...
					x2omi = xomi + xomi;
					x2li = xli + xli;
					xndt = (d2201 * Math.sin(x2omi + xli - g22) + d2211
					...
					xldot = xni + xfact;
					xnddt = (d2201 * Math.cos(x2omi + xli - g22) + d2211
							* Math.cos(xli - g22) + d3210
							* Math.cos(xomi + xli - g32) + d3222
							* Math.cos(-xomi + xli - g32) + d5220
							* Math.cos(xomi + xli - g52) + d5232
							* Math.cos(-xomi + xli - g52) + 2.0 * (d4410
							* Math.cos(x2omi + x2li - g44) + d4422
	...
	function gstime(jdut1) {
		...
		var temp = -6.2e-6 * tut1 * tut1 * tut1 + 0.093104 * tut1 * tut1
				+ (876600.0 * 3600 + 8640184.812866) * tut1 + 67310.54841; // #
																			// sec
		temp = (temp * deg2rad / 240.0) % twopi; // 360/86400 = 1/240, to
													// deg, to rad
	...
	function days2mdhms(year, days) {
		 ...
		 * 
		 * algorithm : set up array for the number of days per month find leap
		 * year - use 1900 because 2000 is a leap year loop through a temp value
		 * while the value is < the days perform int conversions to the correct
		 * day and month convert remainder into h m s using type conversions
		 * 
		 * author : david vallado 719-573-2600 1 mar 2001
		 * 
		 * inputs description range / units year - year 1900 .. 2100 days -
		 * julian day of the year 0.0 .. 366.0
		 * 
		 * outputs : mon - month 1 .. 12 day - day 1 .. 28,29,30,31 hr - hour 0 ..
		 * 23 min - minute 0 .. 59 sec - second 0.0 .. 59.999
		 * 
		 * locals : dayofyr - day of year temp - temporary extended values
		 * inttemp - temporary int value i - index lmonth[12] - int array
		 * containing the number of days per month
		 * 
		 * coupling : none.
		 * ---------------------------------------------------------------------------
		 */
	...
	function jday(year, mon, day, hr, minute, sec) {
		...
		return (367.0
				* year
				- Math
						.floor((7 * (year + Math.floor((mon + 9) / 12.0))) * 0.25)
				+ Math.floor(275 * mon / 9.0) + day + 1721013.5 + ((sec / 60.0 + minute) / 60.0 + hr) / 24.0 // ut
																												// in
																												// days
	...
	function initl(initl_parameters) {
		...
		'use strict';
		var satn = initl_parameters.satn, ecco = initl_parameters.ecco, epoch = initl_parameters.epoch, inclo = initl_parameters.inclo, no = initl_parameters.no, method = initl_parameters.method, opsmode = initl_parameters.opsmode;
		...
		adel = ak
				* (1.0 - del_prime * del_prime - del_prime
	...
	function sgp4init(satrec, sgp4init_parameters) {
		...
		var opsmode = sgp4init_parameters.opsmode, satn = sgp4init_parameters.satn, epoch = sgp4init_parameters.epoch,

		xbstar = sgp4init_parameters.xbstar, xecco = sgp4init_parameters.xecco, xargpo = sgp4init_parameters.xargpo,

		xinclo = sgp4init_parameters.xinclo, xmo = sgp4init_parameters.xmo, xno = sgp4init_parameters.xno,
		...
		// variables, but the user would need to set the satrec.* values first.
		// we
		...
		if (omeosq >= 0.0 || satrec.no >= 0.0) {
			...
			if (Math.abs(cosio + 1.0) > 1.5e-12) {
				satrec.xlcof = -0.25 * j3oj2 * sinio * (3.0 + 5.0 * cosio)
						/ (1.0 + cosio);
			} else {
				satrec.xlcof = -0.25 * j3oj2 * sinio * (3.0 + 5.0 * cosio)
						/ temp4;
			...
			if (satrec.isimp !== 1) {
				...
				satrec.t5cof = 0.2 * (3.0 * satrec.d4 + 12.0 * satrec.cc1
						* satrec.d3 + 6.0 * satrec.d2 * satrec.d2 + 15.0
						* cc1sq * (2.0 * satrec.d2 + cc1sq));
			}
	...
	function twoline2rv(longstr1, longstr2) {
		 ...
		 * 
		 * Provide the two TLE lines as strings `longstr1` and `longstr2`, and
		 * select which standard set of gravitational constants you want by
		 * providing `gravity_constants`:
		 * 
		 * `sgp4.propagation.wgs72` - Standard WGS 72 model
		 * `sgp4.propagation.wgs84` - More recent WGS 84 model
		 * `sgp4.propagation.wgs72old` - Legacy support for old SGP4 behavior
		 * 
		 * Normally, computations are made using various recent improvements to
		 * the algorithm. If you want to turn some of these off and go back into
		 * "afspc" mode, then set `afspc_mode` to `True`.
		 */
		'use strict';
		var opsmode = 'i';
		...
		satrec.nddot = parseFloat("."
				+ parseInt(longstr1.substring(44, 50), 10) + "E"
				+ longstr1.substring(50, 52));
		satrec.bstar = parseFloat("."
				+ parseInt(longstr1.substring(53, 59), 10) + "E"
				+ longstr1.substring(59, 61));
	...
	satellite.propagate = function(satrec, year, month, day, hour, minute,
			second) {
	...
	function sgp4(satrec, tsince) {
		...
		if (satrec.isimp !== 1) {
			...
			tempe = tempe + satrec.bstar * satrec.cc5
					* (Math.sin(mm) - satrec.sinmao);
			templ = templ + satrec.t3cof * t3 + t4
					* (satrec.t4cof + satrec.t * satrec.t5cof);
		}
		nm = satrec.no;
		var em = satrec.ecco;
		inclm = satrec.inclo;
		...
		if (pl < 0.0) {
			...
			mrt = rl * (1.0 - 1.5 * temp2 * betal * satrec.con41) + 0.5 * temp1
					* satrec.x1mth2 * cos2u;
			...
			rvdot = rvdotl + nm * temp1
					* (satrec.x1mth2 * cos2u + 1.5 * satrec.con41) / xke;
	...
	function eci_to_geodetic(eci_coords, gmst) {
		...
		var a = 6378.137;
		var b = 6356.7523142;
		var R = Math.sqrt((eci_coords["x"] * eci_coords["x"])
		...
		var kmax = 20;
		var k = 0;
		var latitude = Math.atan2(eci_coords["z"], Math.sqrt(eci_coords["x"]
				* eci_coords["x"] + eci_coords["y"] * eci_coords["y"]));
		var C;
		while (k < kmax) {
			C = 1 / Math.sqrt(1 - e2
					* (Math.sin(latitude) * Math.sin(latitude)));
			latitude = Math.atan2(eci_coords["z"]
					+ (a * C * e2 * Math.sin(latitude)), R);
			k += 1;
		}
	...
	function eci_to_ecf(eci_coords, gmst) {
		...
		var X = (eci_coords["x"] * Math.cos(gmst))
				+ (eci_coords["y"] * Math.sin(gmst));
		var Y = (eci_coords["x"] * (-Math.sin(gmst)))
				+ (eci_coords["y"] * Math.cos(gmst));
		var Z = eci_coords["z"];
		return {
			x : X,
			y : Y,
	...
	function ecf_to_eci(ecf_coords, gmst) {
		...
		// [Z]eci [0 0 1][Z]ecf
		//
		var X = (ecf_coords["x"] * Math.cos(gmst))
				- (ecf_coords["y"] * Math.sin(gmst));
		var Y = (ecf_coords["x"] * (Math.sin(gmst)))
				+ (ecf_coords["y"] * Math.cos(gmst));
		var Z = ecf_coords["z"];
		return {
			x : X,
			y : Y,
	...
	function geodetic_to_ecf(geodetic_coords) {
		...
		var normal = a
				/ Math
						.sqrt(1 - (e2 * (Math.sin(latitude) * Math
								.sin(latitude))));
	...
	function topocentric_to_look_angles(topocentric) {
		...
		var range_sat = Math.sqrt((top_s * top_s) + (top_e * top_e)
				+ (top_z * top_z));
	...
	satellite.ecf_to_look_angles = function(observer_coords_ecf,
			satellite_coords_ecf) {
		var topocentric_coords = topocentric(observer_coords_ecf,
				satellite_coords_ecf);
	...
	function doppler_factor(my_location, position, velocity) {
		var current_range = Math.sqrt(Math.pow(
				position["x"] - my_location["x"], 2)
		...
		var next_range = Math.sqrt(Math
				.pow(next_pos["x"] - my_location["x"], 2)
	 ...
	 * 
	 * satellite-head.js and satellite-tail.js sandwich the remaining functions
	 * and code in the library headers.
	 * 
	 * This is to separate the satellite.js namespace from the rest of the
	 * javascript environment.
	 * 
